...
def optzf(file_path, add_message):
    ...
    for k in range(0, len(files)):
        ...
        df_af['Комментарий'] = ''
        df_af = df_af[['Артикул (доп)', 'Продукт', 'Склад(Название)', 'В резерве',
                        'Фактический остаток', 'Заказано', 'Оптимальный запас', 'Заказать',
                        'Цена закупа', 'Сумма', 'УП2, ед.'] + months + ['Заказать без обработок', 'Комментарий']]

        # df_af = df_af[df_af['Артикул (доп)'] != 'Итого:']
        ...
        df_af = df_af[['Артикул (доп)', 'Продукт', 'Склад(Название)', 'ОСТАТОК', 'Заказать', 'Цена закупа',
                        'Сумма', 'УП2, ед.', 'Суммарные продажи'] + months +
                        ['Заказать без обработок', 'Комментарий']]
    ...
    if len(pattern_rms) > 1:
        arm_file = next((file for file in pattern_rms if 'Компания РМС ООО_арматура' in file), None)
    ...
    target_folder = r"M:\ForecastNOW\Exch\Orders"
