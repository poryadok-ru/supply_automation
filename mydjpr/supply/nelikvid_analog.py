...
def nelikvids(file_path, porog1, porog2, porog3, porog4, porog5, porog6, add_message):
    ...
    # подтягиваем продажи
    # prk_list.sort()
    df_sez_prod_analog = pd.read_csv(file_path + "Исходники\\сезонность.csv", delimiter=';', dtype='unicode', usecols=['Код', 'Код (доп.)'])
    df_sez_prod_analog['Код (доп.)'] = pd.to_numeric(df_sez_prod_analog['Код (доп.)'], errors='coerce')
    # Сделать первую строку заголовками
    df_prod_analog = df_prod.copy()
    
    df_prod_analog.columns = df_prod_analog.iloc[0]  # присваиваем названия столбцов из первой строки
    df_prod_analog = df_prod_analog[1:].reset_index(drop=True)
    df_prod_analog = df_prod_analog.rename(columns={df_prod_analog.columns[0]: 'Код (доп.)'})
    df_prod_analog = df_prod_analog.merge(df_sez_prod_analog, on='Код (доп.)', how='left')

    files2 = glob.glob(file_path + "Исходники\\Аналоги\\*.csv")
    column_names_analog = ['Главный', 'Код']

    df_analog = pd.read_csv(files2[0], header=None, names=column_names_analog, delimiter=";")
    df_prod_analog = df_prod_analog.merge(df_analog, on='Код', how='left')
    df_prod_analog.loc[df_prod_analog['Главный'].notna(), 'Код'] = df_prod_analog['Главный']

    # ШАГ 1. Найти все столбцы, где есть "Порядок" в названии
    order_columns = [col for col in df_prod_analog.columns if 'Порядок' in str(col)]

    # ШАГ 2. Сгруппировать по "главный" и просуммировать нужные столбцы:
    result = df_prod_analog.groupby('Код')[order_columns].sum().reset_index()

    result = result.merge(df_sez_prod_analog, on='Код', how='left')
    result = result.drop(columns=['Код'])
    result = result.rename(columns={'Код (доп.)': 'Код'})

    
    # df_first_two_rows = result.head(7)
    # result.to_excel("11111.xlsx", index=False)
    ...
    for each_prk in prk_list:
        ...
        # i, j = np.where(result.values == each_prk)
        # column_prk_prod = int(j)
        df_prod_y = result[['Код', each_prk]]
        df_prod_y = df_prod_y.rename(columns={each_prk: 'Продажи 4 мес'})
    ...
    pivot = pd.pivot_table(df_nel_itog, columns='Склад', values=['Остаток текущий, д.е.', 'Неликвиды, д.е'], aggfunc='sum')
    ...
    pivot.to_excel(file_path + 'сводная ' + now1 + '.xlsx')
