...
def dop_ng(file_path, add_message):
    ...
    df_action = pd.read_csv(r"\\lan.sct.ru\x\Воронеж\Подразделения\Коммерческий\Т.Шафиев\Общая\supply\Шафиев\акции2024.csv", delimiter=';', low_memory=False, thousands=" ",
    ...
    for k in range(0, len(files)):
        df_af = pd.read_csv(files[k], delimiter=';', low_memory=False, thousands=" ",
                            usecols=['Артикул(доп.)', 'Наименование', 'В', 'Остаток свободный (Из)', 'ЗакупочнаяЦена',
                                    ...
                                    'Продажи за 12-2024 (В)', 'Суммарные продажи за 7 дней (В)', 'Объем'])
    ...
    df_it = df_it[['Артикул(доп.)', 'Наименование', 'В', 'Остаток свободный (Из)', 'Суммарные продажи за 7 дней (В)',
                'Продажи за 12-2024 (В)', 'Остаток+заказано', 'Количество для перемещения', 'ЗАКАЗ',
    ...
    sheet.column_dimensions['P'].width = 16  # Изменено с Q на P
    sheet.column_dimensions['Q'].width = 16  # Изменено с R на Q
    sheet.column_dimensions['R'].width = 29  # Изменено с S на R
    sheet.column_dimensions['V'].width = 20  # Изменено с W на V
    sheet.column_dimensions['W'].width = 43  # Изменено с X на W
    for row in sheet.iter_rows(max_row=2):
        for cell in row:
            cell.alignment = Alignment(wrap_text=True, vertical='center')

    for i in range(2, (len(df_it)+3)):
        sheet['I'+str(i)].fill = PatternFill(fill_type='solid', start_color='00FFFF99')  # Изменено с J на I

    for i in range(3, (len(df_it)+3)):
        sheet['L'+str(i)] = '=I%s*T%s' % (i, i)  # Изменено с M и U на L и T
        sheet['M' + str(i)] = '=I%s*U%s' % (i, i)  # Изменено с N и V на M и U

    c = str(len(df_it)+2)
    sheet['L1'] = '=SUM(L3:L' + c +')'  # Изменено с M на L
    sheet['M1'] = '=SUM(M3:M' + c +')'  # Изменено с N на M
    sheet['B1'] = 'Коэффициент от продаж 12.24'
    sheet['C1'] = 1
    ...
    rule1.formula = ["$C$1*F3>(G3+H3)"]  # F3, G3, H3 остаются без изменений
    ...
    rule3.formula = ['V3<>""']  # Изменено с W3 на V3
